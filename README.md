...
本项目是基于Java开发的比特币交易所 | BTC交易所 | ETH交易所 | 数字货币交易所 | 交易平台 | 撮合交易引擎。本项目基于SpringCloud微服务开发，可用来搭建和二次开发数字货币交易所，有完整的撮合交易引擎源码、后台管理（后端+前端）、前台（交易页面、活动页面、个人中心等）、原生安卓APP源码(有偿提供）、原生苹果APP源码(有偿提供）、币种钱包RPC源码。
...
搭建起来一个测试站点需要购买好几台云服务器，成本巨大，所以我自己没有搭建测试站，但是系统是完整的，经过了将近一年的商用及实际运营考验。

#### 关于移动端源码与交易机器人源码

Android源码、IOS源码、交易机器人源码有偿提供，需要的加QQ：877070886

Android源码：10000（不包调试安装）、20000（包调试安装）

IOS源码：10000（不包调试安装）、20000（包调试安装）

交易机器人源码：15000（不包调试安装）、30000（包调试安装）
 ...
 **无论是Android、IOS还是机器人的开发，当时都投入很多。** 

 **如果从头开发，起码需要2个安卓、2个IOS开发，并且花费起码两个月时间，即使按照15000/人月来算，也需要投入12万。** 

 **而且新手着手开发APP还需要设计师UI、产品经理等等，前后加起来的投入低于20万是搞不定的。** 

 **所以这个源码的价格是非常优惠的，我不想每个人都解释一下，所以写在这里了。** 

 **注意，你购买到的是源码！！！** 

 **不是市面上不给你源码，仅仅是给你安装包的服务。** 

 **有源码就意味着你对项目有100%的掌控力，你也可以根据业务发展，开发出适合自己业务发展的功能扩展，可以快速的领先市场发展！** 
