...
#### 介绍
基于Java开发的比特币交易所 | BTC交易所 | ETH交易所 | 数字货币交易所 | 交易平台 | 撮合交易引擎。本项目基于SpringCloud微服务开发，可用来搭建和二次开发数字货币交易所，有完整的撮合交易引擎源码、后台管理（后端+前端）、前台（交易页面、活动页面、个人中心等）、安卓APP源码(有偿提供）、苹果APP源码(有偿提供）、币种钱包RPC源码。
...
Android源码及IOS源码有偿提供，需要的加QQ：877070886

Android源码：5000（不包调试安装）、20000（包调试安装）

IOS源码：8000（不包调试安装）、30000（包调试安装）
